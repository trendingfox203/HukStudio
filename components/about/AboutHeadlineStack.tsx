    ...
    <div className="flex flex-col gap-4 lg:col-start-3 lg:row-start-1 lg:mb-32">
      {headlines.map((line, index) => (
        <div key={index} className="flex flex-wrap items-start gap-x-3">
          ...
          {line.tag && (
            <span
              className={`font-mono -ml-1 mt-1 text-[clamp(0.5625rem,0.85vw,0.68rem)] whitespace-nowrap text-ink/50 transition-opacity duration-300 sm:mt-2 ${revealed.has(index) ? "opacity-100" : "opacity-0"
                }`}
            >
              [{line.tag}]
            </span>
